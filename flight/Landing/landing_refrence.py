...
async def move_to(
    ...
    await drone.action.goto_location(latitude, longitude, current_altitude, 0)
    location_reached: bool = False
    # First determine if we need to move fast through waypoints or need to slow down at each one
    # Then loops until the waypoint is reached
    while not location_reached:
        ...
        async for position in drone.telemetry.position():
            ...
            # very precisely checks if location is reached and moves on if so
            if (
                (round(drone_lat, 8) == round(latitude, 8))
                and (round(drone_long, 8) == round(longitude, 8))
                and (round(drone_alt, 3) == round(altitude, 3))
            ):
                location_reached = True
                logging.info("arrived")
                break
